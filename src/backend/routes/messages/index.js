...
routerMessages.get('/info', function(req, res) {
    pool.query('Select messageId,firstname,lastname,patientId,content from Messages JOIN User on Messages.userIdSender=User.userID limit 100', function(err, result, fields) {
        if (err) {
            res.send(err).status(400);
            return;
        }
